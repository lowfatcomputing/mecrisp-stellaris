...
\ ------------------------------------------------
\  Two definitions to scan dictionary on M3/M4
\ ------------------------------------------------

\ Dictionary structure for Cortex M3/4 is:
\  (2-aligned)
\      4 Bytes Link
\      2 Bytes Flags
\  (2-aligned)
...
$4140 $FFC0 opcode? if ." adc"  0 reg. 3 reg. then           \ ADC
...
$A000 $F800 opcode? if ." add"  8 reg. ." pc " 0 imm8. then  \ ADD(5) rd = pc plus immediate
$A800 $F800 opcode? if ." add"  8 reg. ." sp " 0 imm8. then  \ ADD(6) rd = sp plus immediate
...
$D000 $F000 opcode? not if else dup $0F00 and                \ B(1) conditional branch
                       case
                         $0000 of ." beq" endof  \ Z set
                         $0100 of ." bne" endof  \ Z clear
                         $0200 of ." bcs" endof  \ C set
                         $0300 of ." bcc" endof  \ C clear
                       
                         $0400 of ." bmi" endof  \ N set
                         $0500 of ." bpl" endof  \ N clear
                         $0600 of ." bvs" endof  \ V set
                         $0700 of ." bvc" endof  \ V clear
                       
                         $0800 of ." bhi" endof  \ C set Z clear
                         $0900 of ." bls" endof  \ C clear or Z set
                         $0A00 of ." bge" endof  \ N == V
                         $0B00 of ." blt" endof  \ N != V
                       
                         $0C00 of ." bgt" endof  \ Z==0 and N == V
                         $0D00 of ." ble" endof  \ Z==1 or N != V
                         \ $0E00: Undefined Instruction
                         \ $0F00: SWI                       
                       endcase
                       space
